...
* Fix hidden license key in settings #1026
* Fix fatal error when deleting the plugin #1028

= 2.2 (2026-01-05) =

* Require WordPress 6.5 or higher, WooCommerce 9.2 or higher, Polylang 3.7 or higher.
* Use autonomous updater #895
* Add compatibility with the Cost of Goods Sold introduced in WC 9.5 #899.
* Synchronize product brands #996
* Add support for batch REST API for products and terms #994
* Translate custom strings in email previews #930
* Allow to use one email header image per language #910
* Save variation language when saving a variable product #919.
* Fix variations not created when importing an XLIFF file #916.
* Fix draft status of variations created using machine translation #988
* Fix creation of products with shared slugs in REST API #935.
* Fix creation of attribute terms with shared slugs in REST API #915.
* Fix translation of the shipping method when not necessary #901.
* Fix download product link when using one domain per language #902
* Fix Warning: Undefined array key in shop_manager_emails_init() #945
* Fix customer order payment link in back office #954
* Fix products not correctly translated when added directly from the URL #908.
* Fix Thank you page in wrong language if the langue is switched during checkout #903.
* Fix download links when using multiple domains #1002
* Translate WooCommerce Composite products data #1000
* Translate WooCommerce Product bundle sell data #950
* Fix bulk product bundles copy #728.

= 2.1.5 (2025-11-03) =

* Fix WC pages status report #890. See WC issue #53628.
* Fix unnecessary request to the REST API on the checkout page #893.
* Fix the translation to the WooCommerce subscriptions endpoints #904.
* Fix translation of the email sent when a subscription is switched #929
* Fix fatal error: Call to undefined function get_current_screen() with Yith WooCommerce request a quote #940
* Fix variations attribute summaries since WC 10.2 #966
...
* Fix fatal error with WP < 6.7 and WooCommerce Shipment Tracking.

= 2.1.2 (2024-11-12) =

* Add compatibility with WP 6.7
* Fix coupon not correctly applied when using the cart block
* Fix HTML tags removed from all string translations
* Fix empty page search result with Yith Ajax Search

= 2.1.1 (2024-10-07) =

* Fix undesired synchronization of product status
...
* Requires PHP 7.4 or higher, WordPress 6.3 or higher, WC 8.5 or higher
* Deprecate the filter 'pllwc_endpoints_query_vars'
* Improve the integration with Composite Products
* Fix translation of the slug of custom endpoints (added with the filter 'woocommerce_get_query_vars')
* Fix a conflict with payment gateways not compatible with the checkout block
* Fix translation of a shop page using special characters
* Fix attribute terms removed from product when the translation of attributes has been programmatically deactivated
* Fix status report for checkout and cart pages using the WooCommerce classic shortcode block
...
* Fix product page for themes not declaring support to WooCommerce when the language is set from the content
...
* Fix a PHP notice with variations when object cache is active
...
* Use a text area for email additional content translations
...
* Follow-Up emails: Translate categories in additional rule when copying or synchronizing an email
...
* WC 3.3: fix endpoints in nav menu metabox
...
* Fix variations losing their attributes when a translation is trashed
